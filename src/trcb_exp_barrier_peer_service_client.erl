...
-module(trcb_exp_barrier_peer_service_client).
...
%% trcb_exp_barrier_peer_service_client callbacks
-export([start_link/1]).
...
%% gen_server callbacks
init([Socket]) ->
    ?LOG("trcb_exp_barrier_peer_service_client initialized! Node ~p listening to socket ~p", [node(), Socket]),
